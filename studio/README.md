...
- `luma.min` and `luma.max`: the extremes of the measured pixels, which
  ...
  face in a letterboxed frame reported min 0.0). One pixel each, so
  they read on a different basis from every weighted figure above them:
  the pixels the mask MOSTLY selects, weight 0.5 and above
  (`grade/stats.py`'s `MASK_CORE_WEIGHT`), rather than every pixel of
  the mask's support. A feather is a gaussian reaching three sigma, so
  a mask feathered at 0.05 of frame width has a support some 60 pixels
  wider than itself at 400 wide, and on a hard support that outer tail
  (weights around 5e-08, which move the mean by nothing) put the black
  bar's 0.0 straight back on the row. When no pixel reaches 0.5 at all
  (a very soft key, a feather wider than the shape it feathers) the
  extremes come from the mask's own highest weight instead
...
| `mask track CLIP` | `--text "PROMPT"` (repeatable) or `--pick PICK --select IDS` (comma separated ids, or `all`), not both; `--start`, `--end` (seconds); `--steady N` (temporal smoothing frames); `--rotate`; `--wait` (blocks, prints progress to stderr the way `render` does, exits non zero on a `failed` job; a cache hit has no job to wait on and says so on stderr rather than failing); `--force` (clear the frames this request asks for and track them again: a window inside what the matte already covers clears only those frames, a window over its whole span clears the matte, and a window that only PARTLY overlaps it is refused by name rather than clearing anything, since neither reading of it can be right); `--json`; `--port`/`--url`/`--agent`/`--attach` | `POST /api/mask/track`: starts a background SAM track, returns `{"job_id", "mattes": [{"matte_id", "recipe", "state"}, ...], "cached": false, "start_frame", "end_frame"}` immediately unless `--wait`. Cached by clip identity, rotation and recipe, and the cache is a hit only while its answer is still usable: a live job for this recipe, or every frame of the window already written, is free (`{"job_id", "cached": true, "mattes": [...]}`); a HOLE in the window resumes from the first missing frame; a `failed`/`stale`/`cancelled` matte restarts the window; `force` clears the frames the request names and tracks them again, and answers with `cleared_start`, `cleared_end` and `cleared_whole_matte` on EVERY force, so a repair of one second inside a longer matte is tellable from a redo of the whole thing (a narrow force keeps every frame outside its own window) and a force with nothing cached to clear reads as an empty range rather than three missing keys. A force whose window only partly overlaps what the matte declares is refused with a 400 naming both ranges, before anything is deleted: the studio would keep the frames outside the window while the service read the re-track as a different track, which leaves frames on disk that the matte's index cannot describe. The refusal names the two asks that work, the overlap (a repair) and the union (a redo), and a plain track without `force` widens the matte instead and keeps every frame already tracked, with its area, its score and its IoU (the matte store carries those forward across any window of the same picture: the same clip, the same rotation and the same working width). A window that does not overlap the matte at all is refused in its own words rather than pointed at a repair range that does not exist: there is nothing there for force to clear and nothing to repair, so the answer offers the widen and the redo. A force against a matte recorded for another clip, another rotation or another working width is refused as well, by the same three field rule the matte store uses, because clearing part of a matte the store will read as a different track leaves the frames it keeps with no numbers at all. A force while a job for that recipe is still queued or running is refused too, naming the job to cancel: force deletes frames the service's own writer is restoring numbers for. That last one is a check rather than a lock, and the residual window is real: the studio records a job in its own table only after the service has accepted it, so a track queued by another client between this check and the clear is not seen, and the server is threaded, so two forces can be inside the decision at once. Closing it outright means holding a per recipe lock across the cache decision, the clear and the track call, which is a restructure of the route rather than a check, and it has not been done. A resume or a restart answers with `resumed`, `restarted`, `resumed_from` and a `message`, writes back into the SAME matte id, and moves the state to `queued`/`running` again. Before this, an identical retry of a dead track handed back the same dead matte and `job_id: null`, and the only way to get another attempt was to change the words |
...
a caller asking for an absurd preview wants a picture. Every number reaches
that clamp, `?width=0` included (it is announced as a clamp to 1, not silently
read as "no width"); only leaving `width` off, or `?width=` with nothing after
it, means no width was asked for. The answer says which
...
carrying more than 32 components, and a request asking for more than 128 mask
FOLDS between its layers (one per component, one per non zero feather or
finesse blur, and one for a mask that carries no components and still produces
a matte, so the legacy window and key form is not free and a legacy window with
a blur on it costs two). A fold at the feather cap is about 59 ms of numpy at
960x540. Where it is paid differs by half, which is why there are two limits:
the standalone `mask` a measurement is weighted by is folded in numpy on the
thread serving the request, so the per stack cap alone bounds that thread,
while a layer's own mask is folded by ffmpeg in a subprocess or by `gpu.js` in
the browser, so the per request cap bounds the size of that graph and the
number of gaussians a render asks a machine for. The biggest stack in any real
grade here is two components with six in a whole grade, so a request past
either limit is a mistake rather than a grade.
