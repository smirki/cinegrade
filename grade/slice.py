...
# The historical folder, and the fallback when cinegrade is not importable.
# Read it through _slice_dir() below (checkpoint gap 22).
...
def _slice_dir() -> Path:
    """Where a baked Color Slice cube goes (checkpoint gap 22).

    The same run scoped cache root the layer cubes and the generated mattes
    resolve through, so one run's generated files stay in one folder.
    cinegrade is imported lazily and inside the function because cinegrade
    imports THIS module, so importing it back at module level would be a
    cycle; if it cannot be imported at all (this file used on its own), the
    historical grade/luts/slice is the answer, unchanged.
    """
    try:
        import cinegrade as CG                              # noqa: PLC0415
    except ImportError:
        return LUT_SLICE
    return CG.lut_slice_dir()
...
def slice_lut(cfg: dict, size: int = LUT_SIZE) -> Path:
    ...
    d = _slice_dir()                                  # checkpoint gap 22
    d.mkdir(parents=True, exist_ok=True)
    path = d / f"slice_{h}.cube"
    if path.exists():
        return path
    out = bake(cfg, size)
    tmp = d / f".slice_{h}.{os.getpid()}.{threading.get_ident()}.tmp"
    try:
